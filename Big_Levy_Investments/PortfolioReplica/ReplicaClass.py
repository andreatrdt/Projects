...
class PortfolioReplicator:
    ...
    index_components : Dict[str, float]
        Dictionary of index components and their weights.
    var_parameters : Dict[str, float], optional
        Parameters for Value at Risk (VaR) calculation:
            - 'var_confidence': confidence level (e.g., 0.99)
            - 'var_horizon': time horizon in weeks (e.g., 1 for 1 week)
            - 'max_var_threshold': maximum acceptable VaR threshold
    transaction_cost : float, default 0.0
        Transaction cost per trade (as a fraction of the trade value).
    annual_factor : int, default 52
        Annualization factor for returns (e.g., 52 for weekly data).
    """
    ...
    def __init__(
        ...
        *,
        ...
        '''
        Initialize the PortfolioReplicator with target and underlyings data.
        '''
        ...
        self.index_name_str = ', '.join(index_components.keys())
    ...
    ## barr plot correlation with target ##
    ...
    def plot_metrics(self, best_config_normalized, method_title): 
        """
        Plot various metrics of the best configuration.
        Parameters
        ----------
        best_config_normalized : dict
            The best configuration dictionary containing the results.
        method_title : str
            Title of the method used for plotting.
        """ 
        # Plot cumulative returns
        plt.figure(figsize=(12, 6))
        plt.plot(best_config_normalized['cumulative_target'], label=self.index_name_str, color='blue')
        plt.plot(best_config_normalized['cumulative_replica'], label=self.name, color='red')
        plt.title(f"Cumulative returns:  - {self.index_name_str} vs {self.name} - " + method_title)
        plt.xlabel('Date')
        plt.ylabel('Cumulative return')
        plt.legend()
        plt.grid(True)
   ...
   ## run_optuna_normalized
    ...
    def run_equal_weight_portfolio(self, window: int = 0) -> Dict[str, Any]:
        """
        Run an equal-weighted portfolio replication strategy.
        Parameters
        ----------
        window : int, default 0
            Number of initial rows to skip (e.g., for rolling windows).
            If 0, no rows are skipped.
        Returns
        -------
        Dict[str, Any]
            Dictionary containing the results of the equal-weighted portfolio replication.
        """
    ...
    def _optuna_objective_EN(self, trial: optuna.Trial) -> float:
        """
        Objective function for Optuna to optimize ElasticNet parameters.
        Parameters
        ----------
        trial : optuna.Trial
            The Optuna trial object containing the parameters to optimize.
        Returns
        -------
        float
            The negative information ratio (IR) of the replica portfolio.
            Optuna minimizes by default, so we return -IR to maximize it.
        """
    ...
    def extract_target_metrics(self, window) -> Dict[str, Any]:
        ...
        -------
        Dict[str, Any]
            Dictionary containing global performance metrics for the target portfolio.
        """
        ...
        return {
            ...
            'Max Drawdown': f"{float(global_max_drawdown)*100:.2f}%",
            'Tracking Error': "N/A",
...
##############################################################################################################
# ------------------------------------------------------------------------------
# 4. DISCRETE REPLICATION METHODS
# -----------------------------------------------------------------------------
   
    ## compute_discrete_transaction_costs ##
    ...
    ## compute_cumulative_equity_curve ##
    ...
    ## compute_replica_returns_from_weights_and_prices ##
    def compute_replica_returns_from_weights_and_prices(
        self,
        weights_df: pd.DataFrame,           # shape = (T, N)
        prices_df: pd.DataFrame,            # shape = (T, N); index = same dates, columns = same tickers
        tc_rate: float = 0.0                # optional transaction cost per “unit change in weight”
    ) -> pd.Series:
        """
        Build a replica return series from a matrix of weights (T×N) and a prices DataFrame (T×N).
        On each date t:
        * underlying returns R[t,i] = (P[t,i] - P[t-1,i]) / P[t-1,i]
        * if tc_rate > 0, we charge a small cost = tc_rate * sum(|w[t,i] - w[t-1,i]|) on each date t>0
        * replica_return[t] = sum_i( w[t,i] * R[t,i] ) - cost[t]

        Returns a pd.Series of length T, indexed by weights_index.
        """
        weights_matrix = weights_df.to_numpy(dtype=float)  # shape = (T, N)
        weights_index = weights_df.index
        weights_columns = weights_df.columns

        total_cost = 0.0

        # 1) Build weights_df if you were given a raw numpy array
        weights_df = pd.DataFrame(data=weights_matrix, index=weights_index, columns=weights_columns)

        # 2) Verify the indices and columns align exactly
        if not weights_df.index.equals(prices_df.index):
            raise ValueError("weights_df.index must match prices_df.index (dates must align).")

        if list(weights_df.columns) != list(prices_df.columns):
            raise ValueError("weights_df.columns must match prices_df.columns (tickers must align).")

        # 3) Convert to numpy arrays for speed
        W = weights_df.to_numpy(dtype=float)  # shape = (T, N)
        P = prices_df.to_numpy(dtype=float)   # shape = (T, N)
        T, N = W.shape

        # 4) Compute underlying returns matrix R[t,i] = (P[t,i] - P[t-1,i])/P[t-1,i]
        P_prev = np.vstack([P[0], P[:-1]])    # row0 duplicated → P_prev[0]=P[0]
        R = (P - P_prev) / P_prev             # shape = (T, N)
        R[0, :] = 0.0                         # define first‐day returns = 0

        # 5) Initialize an array to hold replica_returns
        r_replica = np.zeros(T, dtype=float)

        # 6) Loop through each t and compute:
        #    a) cost_t = tc_rate * sum(|w[t] - w[t-1]|)  for t>0, cost_0=0
        #    b) r_replica[t] = dot(W[t], R[t]) - cost_t
        last_weight = W[0]
        r_replica[0] = np.dot(W[0], R[0])  # which is zero, since R[0]=0

        for t in range(1, T):
            # a) transaction cost (on change in weights from t-1 → t)
            if tc_rate != 0.0:
                turnover = np.sum(np.abs(W[t] - W[t-1]))
                cost_t = tc_rate * turnover
            else:
                cost_t = 0.0
        ...
        annualized_cost = total_cost / (len(weights_index) / self.annual_factor) if len(weights_index) > 0 else np.nan

        # 7) Return as a Pandas Series
        return pd.Series(data=r_replica, index=weights_df.index, name="replica_returns"), total_cost, annualized_cost


########################################################################### end of the ReplicaClass.py file ######################################################################################################################